package com.luna.hellospock.controller;
...
public class LibraryController {
    ...
    public ResponseEntity<String> isBookAvailable(@PathVariable(name = "isbn") String isbn) {
        return ResponseEntity.ok(
                String.format("%s : 대출 %s", isbn, libraryService.isBookAvailable(isbn) ? "가능" : "불가"));
    }

    @PostMapping("/books/{isbn}/borrow")
    public ResponseEntity<String> borrowBook(@PathVariable(name = "isbn") String isbn) {
