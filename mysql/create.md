...
-- EXISTS를 이용한 조건부 삽입
```
EXISTS clause are faster performance on large datasets, safe null handling, and clear intent. 
It checks if a subquery returns any rows and stops instantly at the first match.
```
```sql
